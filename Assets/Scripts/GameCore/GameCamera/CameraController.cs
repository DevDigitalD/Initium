...
namespace GameCore.GameCamera
{
    public class CameraController : MonoBehaviour
    {
        ...
        private float _initDist = 42f; // var for calculation [used in Pinching()]
        private float _initOrtho = 6;  // var for calculation [used in Pinching()]
        ...
        private void Pinching()
        {
            if (Input.touchCount > 1)
            {
                ...
                if (!_lastFramePinch)
                {
                    _zoomTarget = _cameraToMove.ScreenToWorldPoint((touchZero.position + touchOne.position) / 2);
                    _initPos = _cameraToMove.transform.position;
                    _initDist = Vector2.Distance(touchZero.position, touchOne.position);
                    _initOrtho = _cameraToMove.orthographicSize;
                }
                ...
                if (touchZero.phase == TouchPhase.Moved || touchOne.phase == TouchPhase.Moved)
                {
                    ...
                    if (_initOrtho != _orthoMin)
                    {
                        float a = -(1 / ((_initOrtho - _orthoMin)));
                        float b = 1 + (_orthoMin / ((_initOrtho - _orthoMin)));
                        t = Mathf.Clamp(a * x + b, 0f, 1f);

                        _cameraToMove.transform.position = Vector3.Lerp(_initPos, new Vector3(_zoomTarget.x, _zoomTarget.y, _cameraToMove.transform.position.z), t);
                ...
                _zoomTarget = _cameraToMove.ScreenToWorldPoint(_cameraToMove.WorldToScreenPoint(_zoomTarget) - (targ - prevTarg));
                _initPos = _cameraToMove.ScreenToWorldPoint(_cameraToMove.WorldToScreenPoint(_initPos) - (targ - prevTarg));
            }
            else
            {
